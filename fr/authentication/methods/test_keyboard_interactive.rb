...
module Authentication
  module Methods
	...
	transport.expect do |t,packet|
	  assert_equal USERAUTH_REQUEST, packet.type
	  t.return(USERAUTH_INFO_REQUEST, :string, "", :string, "", :long, 1, :string, "Password:", :bool, false)
	...
	assert_equal false, subject.authenticate("ssh-connection", "jamis", "the-password")
      ...
      def test_authenticate_should_duplicate_password_as_needed_to_fil_request
	transport.expect do |t,packet|
	  assert_equal USERAUTH_REQUEST, packet.type
	  t.return(USERAUTH_INFO_REQUEST, :string, "", :string, "", :long, 2, :string, "Password:", :bool, false, :string, "Again:", :bool, false)
	  t.expect do |t2,packet2|
	    assert_equal USERAUTH_INFO_RESPONSE, packet2.type
	...
	transport.expect do |t,packet|
	  assert_equal USERAUTH_REQUEST, packet.type
	  t.return(USERAUTH_INFO_REQUEST, :string, "", :string, "", :string, "", :long, "Name:", :bool, true, :string, "Password", :bool, false)
	  t.expect do |t2,packet2|
	    assert_equal USERAUTH_INFO_RESPONSE< packet2.type
	    assert_equal 2, packet2.read_long
	...
	prompt.expects(:_ask).with("Name:", anythng, true).returns("name")
	...
	transport.expect do |t,packet|
      ...
      def subject(options={})
        @subject ||= Net::SSH::Authenticateion::Methods::KeyboardINteractive.new(session(options), options)
      end
