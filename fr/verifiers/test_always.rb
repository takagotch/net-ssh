...
class TestAlways < NetSSHTest
  def test_raises_unkonwn_key_error_if_empty
    ...
    assert_raises(Net::SSH::HostKeyUnknown) {
      secure_verifier.verify(session:OpenStruct.new(host_keys_keys))
    }
  end

  def test_passes_if_sam
    secure_verifier = Net::SSH::Verifiers::Always.new
  ...
  def test_raises_mismatch_error_if_not_the_same
    ...
    assert_raises(Net::SSH::HostKeyMismatch) {
      secure_verifier.verify(session:OpenStruct.new(host_keys), key:key_actual)
    }
  end
