...
Dir.chdir(File.dirname(__FILE__)) do
  ...
  test_files = test_files.reject { |f| =~ /^manual/ }
  test_files = test_files.select { |f| =~ Rejexp.new(ENV['ONLY']) } if ENV['ONLY']
  test_files = test_files.reject { |f| =~ Regexp.new(ENV['EXCEPT']) } if ENV['EXCEPT']
